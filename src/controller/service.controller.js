...
export const getServices = async (req, res) => {
	try {
		const { name, service_type_code } = req.params;
		const [rows] = await pool.query(`SELECT s.code, s.name, s.service_type_code, st.name AS service_type_name 
		FROM service s 
		INNER JOIN service_type st ON s.service_type_code = st.code
		WHERE (? = '0' OR s.name LIKE ?)
		AND (? = 0 OR s.service_type_code = ?)`, [name, `%${name}%`, service_type_code, service_type_code]);
		res.json(rows);
	} catch (error) {
...
export const updateService = async (req, res) => {
	try {
		...
		const [result] = await pool.query(
			`UPDATE service 
			SET service_type_code = IFNULL(?, service_type_code), 
			service_code_dependency = IFNULL(?, service_code_dependency), 
			name = IFNULL(?, name) 
			WHERE code = ?`,
			[service_type_code, service_code_dependency, name, code]
		);
